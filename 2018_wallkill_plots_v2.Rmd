...
chem.wallkill.old <- read.csv("data/Wallkill_2018/wallkill_chem_ras-2017-2018_peers-2018.csv", stringsAsFactors = FALSE,
...
# flow.qual.wallkill18 <- readxl::read_excel("data/Wallkill_2018/Wallkill_2018_flow_fieldsheets.xlsx", sheet = "simplified qual")
# flow.qual.wallkill18$sample_date <- as.Date(flow.qual.wallkill18$sample_date)
# flow.qual.wallkill18$project_name <- "wallkill special study 2018"

flow.quant.wallkill18 <- readxl::read_excel("data/Wallkill_2018/Wallkill_2018_flow_fieldsheets.xlsx", sheet = "flow") %>%
  select(station, sample_date, flow_qual, flow_quant_cfs) %>% 
  mutate(station = tolower(station))
flow.quant.wallkill18$sample_date <- as.Date(flow.quant.wallkill18$sample_date)
flow.quant.wallkill18$project_name <- "wallkill special study 2018"

chem.wallkill$sample_date <- as.Date(chem.wallkill$date_time)
# chem.wallkill <- chem.wallkill %>% 
#   left_join(flow.qual.wallkill18, by = c('sample_date', 'project_name')) %>% 
...
# Looking into why so much 2018 data without flow...
chem.wallkill.2018noflow <- chem.wallkill %>% 
  filter(project_name == "wallkill special study 2018") %>% 
  filter(flow_qual == "not observed") %>% 
  mutate(site_date = paste0(station,"_",sample_date))
...
# Load plot
ggplot()+
  geom_point(data = chem.wallkill.TP, aes(x = station, y = load_kg_per_day, shape = flow_qual, color = project_name), size = 2) +
  # geom_boxplot(data = chem.basin13.TP, aes("_Basin 13 historic",Result_Val)) +
  scale_color_manual(values = c("wallkill peers 2018" = "green", "wallkill special study 2017" = "black", "wallkill special study 2018" = "black")) +
